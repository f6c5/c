...
// artýk bellekte yer tahsis edilmiþtir.
head -> data = 1;
head -> next = NULL;
/* listeye yeni eleman ekleme */
/* C++'ta head -> next = new node() þeklinde kullanýlabilir. */
...
int counter = 0;
while(head != NULL) { // head->next!=NULL koþulu olsaydý son düðüm sayýlmazdý
counter++;
head = head -> next;
}
return counter;
...
if(head -> data == key) { // ilk düðüm silinecek mi diye kontrol ediliyor.
...
struct node *temp2;
while(head!= NULL) { // while içindeki koþul temp2 -> next, NULL deðilse
temp2=head;
head = head->next;
free(temp2);
}
